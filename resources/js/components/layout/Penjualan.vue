                            ...
                            <table class="table table-striped table-hover">
                                ...
                                <tbody>
                                    <tr v-bind:key="detail['id']" v-for="detail in filtereddetail">
                                        ...
                                        <td v-if="update_sparepart == false" >{{detail.Jumlah}}</td>
                                        <td v-else>
                                            <div class="input-group mb-4">
                                                ...
                                                <div class="input-group-append">
                                                    <button class="btn btn-success" @click="updatespareparthandler(detail)" 
                                                    data-title="Update_Sparepart">
                                        ...
                                        <td class="text-center">
                                            <p data-placement="top" data-toggle="tooltip" title="Edit">
                                                <button class="btn btn-primary" @click="updatespareparttoggler(),getalldetailpenjualan()">
                                                    <i class="fas fa-edit"></i>
                                                </button>
                                            </p>
                                        </td>
...
export default {
    ...
    methods:{
        ...
        updatespareparttoggler()
        {
            this.update_sparepart = !this.update_sparepart;
        },
        updatespareparthandler(detail)
        {
            this.Detail_Sparepart = detail;
            this.Detail_Sparepart.Subtotal_Pengadaan = detail.Jumlah * detail.Harga_Satuan;
            console.log(detail)
            this.updatesparepart(detail.Id_Detail_Sparepart);
        },
        async updatesparepart (id) {
            try {
                const payload = {
                    Jumlah : this.Detail_Sparepart.Jumlah
                }
                await penjualanController.updatesparepart(payload,id)
                this.getallpenjualan()
                this.getalldetailpenjualan()
                this.getalldetailjasa()
                this.refresh()
